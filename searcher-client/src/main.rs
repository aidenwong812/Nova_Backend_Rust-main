...
use std::sync::Arc;
use  searcher_client::{   operate_db, return_nft_transaction_data, return_token_swap_data, transcation_datas_routes, websocket_run};
...
async fn main()  {
    ...
    tokio::spawn(async move {
        return_token_swap_data(token_rx, conn1).await;
    });
}